 ...
 *
 *  - THESE MAY BE RELATED..HOLDING DEVICE AT AN ANGLE ACCELERATION-Y NEVER OVERCOMES ACCELERATION-Z (GUESSING EARTH-G)
 *    - #1C: WHILE VERTICAL MOTION "BOBBLE" IS GOOD, IT MIGHT BE BETTER USING MAX ACCELERATION-YZ DELTA
 *    - MAYBE A Y-SCALING FACTOR APPLIED IN update() FOR EASIER VERTICAL BOUNCE, SHAKING A DEVICE UP-DOWN ON Y-AXIS
 *
 ...
 *    - http://stackoverflow.com/questions/25744984/implement-a-kalman-filter-to-smooth-data-from-deviceorientation-api
 *
 *
 *
 *  NOTES:
...
var
  ...
  PIGGLY = (function() {
    ...
    function _scaleCanvas() {
      ...
      // _GAME.renderer.renderSession.roundPixels = false;
        ...
        PLAY: {
          ...
          create: function(){
            ...
            if (SETTINGS.PIGGLY.show_text) {
              ...
              delta_y = ((SPRITE.bobble.obj.height + gap_y + SPRITE.text_main.obj.height) - SPRITE.bobble.obj.height) / 2;
              ...
              this.make.tween(SPRITE.text_main.obj).to({ alpha: 1 }, 1000, Phaser.Easing.Linear.None, true, 1500);
            }

            this.make.tween(SPRITE.bobble.obj).to({ alpha: 1 }, 1000, Phaser.Easing.Linear.None, true, 1500);
            ...
            if (SENSORS.getAccelerationGravityXYZ() === 0) {
              SENSORS.shutDown();
              SPRITE.base.obj = this.add.sprite(SPRITE.bobble.obj.x, SPRITE.bobble.obj.y, _R.PIGGLY_SPRITESHEET.id, SPRITE.base.id);
            }
            else
              SPRITE.base.obj = this.add.sprite(SPRITE.bobble.obj.x, SPRITE.bobble.obj.y + SPRITE.bobble.obj.height * SPRITE.base.gravity_offset, _R.PIGGLY_SPRITESHEET.id, SPRITE.base.id);
            ...
            if (SETTINGS.PIGGLY.enable_keyboard || SETTINGS.PIGGLY.enable_accelerometer && SENSORS.isReady()) {
              ...
              if (SETTINGS.PIGGLY.enable_keyboard) {
                if (_KEYBOARD.left.isDown && !_KEYBOARD.right.isDown)
                  acceleration_X = SETTINGS.SPRING_LINEAR.force_max;
